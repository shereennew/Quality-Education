...
// Fetch classroom details[cite: 3]
...
// Fetch students in this classroom for the dropdown filter[cite: 3]
...
// Fetch questions/quizzes for this setup
...
foreach ($rows as $row) {
    ...
    if ($status === 'Completed') $status = 'Correct';
...
<body class="bg-pastel-bg text-pastel-text min-h-screen font-sans flex flex-col">
        ...
        <div class="max-w-7xl mx-auto px-6 py-3.5 flex justify-between items-center">
            <div class="flex items-center space-x-4">
                <a href="classroom.php?id=<?php echo $class_id; ?>"
                    class="text-pastel-text hover:text-pastel-hover bg-white/60 hover:bg-white px-3 py-1.5 rounded-lg text-xs font-semibold transition border border-blue-100">&larr; Back to Classroom</a>
                <h1 class="text-base font-bold text-pastel-text tracking-wide">
                    Question-by-Question Student Answers & Analytics
                </h1>
            </div>
        </div>
    </header>
    ...
    <main class="max-w-7xl mx-auto px-6 py-8 space-y-6 flex-1 w-full">
        <!-- Filter Bar -->
        <section class="bg-pastel-card rounded-2xl shadow-sm border border-blue-100 p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h2 class="text-base font-bold text-pastel-text">Filter Responses by Student</h2>
                <p class="text-xs text-slate-400 mt-0.5">Showing questions, charts, and individual answers for <?php echo htmlspecialchars($current_class['name']); ?>[cite: 3].</p>
            </div>
            ...
            <form method="GET" class="flex items-center gap-2 w-full sm:w-auto">
                ...
                <?php if ($student_filter > 0): ?>
                    <a href="quiz_summary.php?classroom_id=<?php echo $class_id; ?>" class="text-xs text-slate-400 hover:text-slate-600 underline whitespace-nowrap">Reset</a>
                <?php endif; ?>
            </form>
        </section>
        ...
        <div class="space-y-6">
            <?php if (count($questions_data) > 0): ?>
                <?php foreach ($questions_data as $q_id => $q): ?>
                    <section class="bg-pastel-card rounded-2xl shadow-sm border border-blue-100 overflow-hidden">
                        ...
                        <div class="p-6 border-b border-blue-100 bg-pastel-bg/30">
                            <span class="text-xs font-bold uppercase tracking-wider text-pastel-primary">
                                <?php echo htmlspecialchars($q['chapter_name']); ?> <?php echo !empty($q['subtopic_name']) ? ' / ' . htmlspecialchars($q['subtopic_name']) : ''; ?>
                            </span>
                            <h3 class="text-base font-bold text-pastel-text mt-1">
                                <?php echo htmlspecialchars($q['question']); ?>
                            </h3>
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-4 text-xs">
                                <div class="bg-white p-2 rounded-lg border border-blue-50"><b>A:</b> <?php echo htmlspecialchars($q['option_a']); ?></div>
                                <div class="bg-white p-2 rounded-lg border border-blue-50"><b>B:</b> <?php echo htmlspecialchars($q['option_b']); ?></div>
                                <div class="bg-white p-2 rounded-lg border border-blue-50"><b>C:</b> <?php echo htmlspecialchars($q['option_c']); ?></div>
                                <div class="bg-white p-2 rounded-lg border border-blue-50"><b>D:</b> <?php echo htmlspecialchars($q['option_d']); ?></div>
                            </div>
                        </div>
                        ...
                        <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-6 border-b border-blue-100 items-center bg-white">
                            <div class="flex flex-col items-center justify-center">
                                <span class="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Response Distribution</span>
                            ...
                            <div class="md:col-span-2 grid grid-cols-2 sm:grid-cols-4 gap-4">
                                <div class="bg-emerald-50 border border-emerald-100 p-4 rounded-xl text-center">
                                    ...
                                    <span class="text-xl font-bold text-emerald-700 mt-1 inline-block"><?php echo $q['stats']['Correct']; ?></span>
                                </div>
                                <div class="bg-rose-50 border border-rose-100 p-4 rounded-xl text-center">
                                    ...
                                    <span class="text-xl font-bold text-rose-700 mt-1 inline-block"><?php echo $q['stats']['Incorrect']; ?></span>
                                </div>
                                <div class="bg-amber-50 border border-amber-100 p-4 rounded-xl text-center">
                                    ...
                                    <span class="text-xl font-bold text-amber-700 mt-1 inline-block"><?php echo $q['stats']['Attempted']; ?></span>
                                </div>
                                <div class="bg-slate-50 border border-slate-200 p-4 rounded-xl text-center">
                                    ...
                                    <span class="text-xl font-bold text-slate-700 mt-1 inline-block"><?php echo $q['stats']['Pending']; ?></span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-left border-collapse">
                                <thead>
                                    <tr class="text-slate-400 text-xs font-semibold uppercase tracking-wider border-b border-blue-50 bg-pastel-bg/20">
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <div class="bg-pastel-card rounded-2xl shadow-sm border border-blue-100 p-8 text-center text-slate-400 text-sm">

...
// Group materials into Main Topic vs Subtopics
$main_materials = [];
$subtopic_materials = [];
foreach ($materials as $mat) {
    if (!empty($mat['subtopic_name'])) {
        $subtopic_materials[$mat['subtopic_name']][] = $mat;
    } else {
        $main_materials[] = $mat;
    }
}
...
// Group subtopic quizzes by subtopic name
$grouped_sub_quizzes = [];
foreach ($sub_quizzes as $q) {
    if (!empty($q['subtopic_name'])) {
        $grouped_sub_quizzes[$q['subtopic_name']][] = $q;
    }
}
?>
<!DOCTYPE html>
...
<body class="bg-pastel-bg text-pastel-text min-h-screen font-sans flex flex-col">
    ...
    <main class="max-w-5xl mx-auto px-6 py-8 space-y-8 flex-1 w-full">
        ...
        <section class="bg-pastel-card rounded-2xl shadow-sm border border-blue-100 p-6 space-y-6">
            ...
            <!-- Main Topic Materials -->
            <div>
                <h3 class="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Main Topic Materials</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <?php if (count($main_materials) > 0): ?>
                        <?php foreach ($main_materials as $mat): ?>
                            <div class="border border-blue-100 rounded-xl p-4 bg-pastel-bg/20 flex flex-col justify-between space-y-3">
                                <div>
                                    <div class="flex justify-between items-start">
                                        <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Main Topic Material</span>
                                        <button onclick="toggleEditModal('edit-material-<?php echo $mat['id']; ?>')" class="text-xs text-pastel-primary hover:text-pastel-hover font-semibold">Edit</button>
                                    </div>
                                    <h3 class="text-sm font-bold text-pastel-text mt-0.5"><?php echo htmlspecialchars($mat['title']); ?></h3>
                                </div>
                                <?php if (!empty($mat['file_path'])): ?>
                                    <div>
                                        <a href="../../<?php echo htmlspecialchars($mat['file_path']); ?>" target="_blank" class="inline-block bg-pastel-primary hover:bg-pastel-hover text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition">
                            ...
                            <div id="edit-material-<?php echo $mat['id']; ?>" class="hidden fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
                                <div class="bg-white rounded-2xl p-6 max-w-md w-full space-y-4 border border-blue-100 shadow-lg">
                                    ...
                                    <form action="chapter_details.php?chapter=<?php echo urlencode($chapter_name); ?>" method="POST" enctype="multipart/form-data" class="space-y-3">
                                        <input type="hidden" name="action" value="edit_material">
                                        <input type="hidden" name="material_id" value="<?php echo $mat['id']; ?>">
                                        <div>
                                            <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Assign to Subtopic (Optional)</label>
                                            <select name="subtopic_name" class="w-full text-xs px-3 py-2 rounded-xl border border-blue-100 bg-pastel-bg/40">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" name="material_title" required value="<?php echo htmlspecialchars($mat['title']); ?>" class="w-full text-xs px-3 py-2 rounded-xl border border-blue-100 bg-pastel-bg/40">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Replace File (Optional)</label>
                                            <input type="file" name="material_file" class="w-full text-xs text-slate-500">
                                        </div>
                                        <div class="flex justify-end space-x-2 pt-2">
                                            <button type="button" onclick="toggleEditModal('edit-material-<?php echo $mat['id']; ?>')" class="px-3 py-1.5 text-xs bg-slate-100 rounded-xl">Cancel</button>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <p class="text-xs text-slate-400 col-span-2">No main topic materials found.</p>
                    <?php endif; ?>
                </div>
            </div>

            <!-- Subtopic Grouped Materials -->
            <?php foreach ($subtopic_materials as $sub_name => $sub_ats): ?>
                <div class="border-t border-blue-100 pt-4 mt-4">
                    <h3 class="text-xs font-bold uppercase tracking-wider text-pastel-primary mb-3">Subtopic: <?php echo htmlspecialchars($sub_name); ?></h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <?php foreach ($sub_ats as $mat): ?>
                            <div class="border border-blue-100 rounded-xl p-4 bg-pastel-bg/20 flex flex-col justify-between space-y-3">
                                <div>
                                    <div class="flex justify-between items-start">
                                        <span class="text-[10px] font-bold uppercase tracking-wider text-pastel-primary">
                                            <?php echo htmlspecialchars($mat['subtopic_name']); ?>
                                        </span>
                                        <button onclick="toggleEditModal('edit-material-<?php echo $mat['id']; ?>')" class="text-xs text-pastel-primary hover:text-pastel-hover font-semibold">Edit</button>
                                    </div>
                                    <h3 class="text-sm font-bold text-pastel-text mt-0.5"><?php echo htmlspecialchars($mat['title']); ?></h3>
        ...
        <section class="bg-pastel-card rounded-2xl shadow-sm border border-blue-100 p-6 space-y-6">
            ...
            <div>
                ...
                <?php if (empty($grouped_sub_quizzes)): ?>
                    <p class="text-sm text-slate-500 italic bg-white p-4 rounded-xl border border-slate-100">No subtopic
                        quizzes available.</p>
                <?php else: ?>
                    <?php foreach ($grouped_sub_quizzes as $sub_name => $questions): ?>
                        <div class="mb-6 bg-pastel-bg/30 p-4 rounded-2xl border border-blue-100">
                            <h3 class="text-sm font-bold text-pastel-primary mb-3 uppercase tracking-wide">Subtopic: <?php echo htmlspecialchars($sub_name); ?></h3>
                            <div class="space-y-4">
